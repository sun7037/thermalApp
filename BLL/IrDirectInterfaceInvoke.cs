...
namespace BLL {
    ...
    {
        uint counter;     /*!< 对于每个接收到的帧进行连续编号 */
        uint counterHW;   /*!< 从设备接收的硬件计数器 */
        long timestamp;      /*!< 时间戳以单位为单位（每秒10000000个单位）。 */
        long timestampMedia;
        EvoIRFlagState flagState; /*!< 在拍摄时快门标志的状态 */
        float tempChip;           /*!< 芯片温度 */
        float tempFlag;           /*!< Shutter flag temperature快门标志温度 */
        float tempBox;            /*!< Temperature inside camera housing   相机内部温度。*/
    }

    internal class IrDirectInterfaceInvoke {
        /**
         * @brief Initializes an IRImager instance connected to this computer via USB  brief 初始化通过 USB 连接到此计算机的 IRImager 实例
        ...
        /**
         * @brief sets palette scaling method for multiple cameras设置多个相机的调色板缩放方法
         * Defined in IRImager Direct-SDK, see
         * enum EnumOptrisPaletteScalingMethod{eManual = 1,
         *                                     eMinMax = 2,
         *                                     eSigma1 = 3,
        ...
        /**
        * @brief Set the clipped format position. For PI1M only position with x=0 are valid.
        * @param[in] camId Camera instance id from init to apply this function
        ...
        */
        [DllImport("libirimager.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int evo_irimager_multi_set_clipped_format_position(uint camId, ushort x, ushort y);

        /**
        * @brief Get the clipped format position. Returns uint max if not set previously.
        * @param[in] camId Camera instance id from init to apply this function
        ...
        */
        [DllImport("libirimager.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int evo_irimager_multi_get_clipped_format_position(uint camId, out ushort x, out ushort y);
